...
pub fn decodeInstrAt(pc: u16, gb: *Gb) Instr {
    const opcode = mem.read(gb, pc);

    const opcodeReg = Src8.decode(@truncate(opcode));

    return switch (opcode) {
        0x00 => .NOP,
        0x01 => Instr{ .LD_16 = .{ .dst = .BC, .src = Src16{ .Imm = as16(mem.read(gb, pc + 2), mem.read(gb, pc + 1)) } } },
        0x02 => Instr{ .LD_8 = .{ .dst = .IndBC, .src = .A } },
        0x03 => Instr{ .INC_16 = Dst16.BC },
        0x04 => Instr{ .INC_8 = Dst8.B },
        0x05 => Instr{ .DEC_8 = Dst8.B },
        0x06 => Instr{ .LD_8 = .{ .dst = .B, .src = Src8{ .Imm = mem.read(gb, pc + 1) } } },
        0x07 => .RLCA,
        0x08 => Instr{ .LD_16 = .{ .dst = Dst16{ .Ind = as16(mem.read(gb, pc + 2), mem.read(gb, pc + 1)) }, .src = .SP } },
        0x09 => Instr{ .ADD_16 = .{ .dst = .HL, .src = .BC } },
        0x0a => Instr{ .LD_8 = .{ .dst = .A, .src = .IndBC } },
        0x0b => Instr{ .DEC_16 = .BC },
        0x0c => Instr{ .INC_8 = .C },
        0x0d => Instr{ .DEC_8 = .C },
        0x0e => Instr{ .LD_8 = .{ .dst = .C, .src = Src8{ .Imm = mem.read(gb, pc + 1) } } },
        0x0f => .RRCA,

        0x10 => .STOP,
        0x11 => Instr{ .LD_16 = .{ .dst = .DE, .src = Src16{ .Imm = as16(mem.read(gb, pc + 2), mem.read(gb, pc + 1)) } } },
        0x12 => Instr{ .LD_8 = .{ .dst = .IndDE, .src = .A } },
        0x13 => Instr{ .INC_16 = Dst16.DE },
        0x14 => Instr{ .INC_8 = Dst8.D },
        0x15 => Instr{ .DEC_8 = Dst8.D },
        0x16 => Instr{ .LD_8 = .{ .dst = .D, .src = Src8{ .Imm = mem.read(gb, pc + 1) } } },
        0x17 => .RLA,
        0x18 => Instr{ .JR = mem.read(gb, pc + 1) },
        0x19 => Instr{ .ADD_16 = .{ .dst = .HL, .src = .DE } },
        0x1a => Instr{ .LD_8 = .{ .dst = .A, .src = .IndDE } },
        0x1b => Instr{ .DEC_16 = .DE },
        0x1c => Instr{ .INC_8 = .E },
        0x1d => Instr{ .DEC_8 = .E },
        0x1e => Instr{ .LD_8 = .{ .dst = .E, .src = Src8{ .Imm = mem.read(gb, pc + 1) } } },
        0x1f => .RRA,

        0x20 => Instr{ .JR_COND = .{ .offset = mem.read(gb, pc + 1), .cond = .NZ } },
        0x21 => Instr{ .LD_16 = .{ .dst = .HL, .src = Src16{ .Imm = as16(mem.read(gb, pc + 2), mem.read(gb, pc + 1)) } } },
        0x22 => Instr{ .LD_8 = .{ .dst = .IndHLInc, .src = .A } },
        0x23 => Instr{ .INC_16 = .HL },
        0x24 => Instr{ .INC_8 = .H },
        0x25 => Instr{ .DEC_8 = .H },
        0x26 => Instr{ .LD_8 = .{ .dst = .H, .src = Src8{ .Imm = mem.read(gb, pc + 1) } } },
        0x27 => .DAA,
        0x28 => Instr{ .JR_COND = .{ .offset = mem.read(gb, pc + 1), .cond = .Z } },
        0x29 => Instr{ .ADD_16 = .{ .dst = .HL, .src = .HL } },
        0x2a => Instr{ .LD_8 = .{ .dst = .A, .src = .IndHLInc } },
        0x2b => Instr{ .DEC_16 = .HL },
        0x2c => Instr{ .INC_8 = .L },
        0x2d => Instr{ .DEC_8 = .L },
        0x2e => Instr{ .LD_8 = .{ .dst = .L, .src = Src8{ .Imm = mem.read(gb, pc + 1) } } },
        0x2f => .CPL,

        0x30 => Instr{ .JR_COND = .{ .offset = mem.read(gb, pc + 1), .cond = .NC } },
        0x31 => Instr{ .LD_16 = .{ .dst = .SP, .src = Src16{ .Imm = as16(mem.read(gb, pc + 2), mem.read(gb, pc + 1)) } } },
        0x32 => Instr{ .LD_8 = .{ .dst = .IndHLDec, .src = .A } },
        0x33 => Instr{ .INC_16 = .SP },
        0x34 => Instr{ .INC_8 = .IndHL },
        0x35 => Instr{ .DEC_8 = .IndHL },
        0x36 => Instr{ .LD_8 = .{ .dst = .IndHL, .src = Src8{ .Imm = mem.read(gb, pc + 1) } } },
        0x37 => .SCF,
        0x38 => Instr{ .JR_COND = .{ .offset = mem.read(gb, pc + 1), .cond = .C } },
        0x39 => Instr{ .ADD_16 = .{ .dst = .HL, .src = .SP } },
        0x3a => Instr{ .LD_8 = .{ .dst = .A, .src = .IndHLDec } },
        0x3b => Instr{ .DEC_16 = .SP },
        0x3c => Instr{ .INC_8 = .A },
        0x3d => Instr{ .DEC_8 = .A },
        0x3e => Instr{ .LD_8 = .{ .dst = .A, .src = Src8{ .Imm = mem.read(gb, pc + 1) } } },
        0x3f => .CCF,
        ...
        0xc1 => Instr{ .POP = .BC },
        0xc2 => Instr{ .JP_COND = .{ .addr = as16(mem.read(gb, pc + 2), mem.read(gb, pc + 1)), .cond = .NZ } },
        0xc3 => Instr{ .JP = as16(mem.read(gb, pc + 2), mem.read(gb, pc + 1)) },
        0xc4 => Instr{ .CALL_COND = .{ .addr = as16(mem.read(gb, pc + 2), mem.read(gb, pc + 1)), .cond = .NZ } },
        0xc5 => Instr{ .PUSH = .BC },
        0xc6 => Instr{ .ADD = Src8{ .Imm = mem.read(gb, pc + 1) } },
        0xc7 => Instr{ .RST = 0x00 },
        0xc8 => Instr{ .RET_COND = .Z },
        0xc9 => .RET,
        0xca => Instr{ .JP_COND = .{ .addr = as16(mem.read(gb, pc + 2), mem.read(gb, pc + 1)), .cond = .Z } },
        0xcb => blk: {
            const opcodeCb = mem.read(gb, pc + 1);
        ...
        0xcc => Instr{ .CALL_COND = .{ .addr = as16(mem.read(gb, pc + 2), mem.read(gb, pc + 1)), .cond = .Z } },
        0xcd => Instr{ .CALL = as16(mem.read(gb, pc + 2), mem.read(gb, pc + 1)) },
        0xce => Instr{ .ADC = Src8{ .Imm = mem.read(gb, pc + 1) } },
        0xcf => Instr{ .RST = 0x08 },

        0xd0 => Instr{ .RET_COND = .NC },
        0xd1 => Instr{ .POP = .DE },
        0xd2 => Instr{ .JP_COND = .{ .addr = as16(mem.read(gb, pc + 2), mem.read(gb, pc + 1)), .cond = .NC } },
        0xd3 => Instr{ .INVALID = opcode },
        0xd4 => Instr{ .CALL_COND = .{ .addr = as16(mem.read(gb, pc + 2), mem.read(gb, pc + 1)), .cond = .NC } },
        0xd5 => Instr{ .PUSH = .DE },
        0xd6 => Instr{ .SUB = Src8{ .Imm = mem.read(gb, pc + 1) } },
        0xd7 => Instr{ .RST = 0x10 },
        0xd8 => Instr{ .RET_COND = .C },
        0xd9 => .RETI,
        0xda => Instr{ .JP_COND = .{ .addr = as16(mem.read(gb, pc + 2), mem.read(gb, pc + 1)), .cond = .C } },
        0xdb => Instr{ .INVALID = opcode },
        0xdc => Instr{ .CALL_COND = .{ .addr = as16(mem.read(gb, pc + 2), mem.read(gb, pc + 1)), .cond = .C } },
        0xdd => Instr{ .INVALID = opcode },
        0xde => Instr{ .SBC = Src8{ .Imm = mem.read(gb, pc + 1) } },
        0xdf => Instr{ .RST = 0x18 },

        0xe0 => Instr{ .LD_8 = .{ .dst = Dst8{ .IndIoReg = mem.read(gb, pc + 1) }, .src = .A } },
        0xe1 => Instr{ .POP = .HL },
        0xe2 => Instr{ .LD_8 = .{ .dst = .IndC, .src = .A } },
        0xe3 => Instr{ .INVALID = opcode },
        0xe4 => Instr{ .INVALID = opcode },
        0xe5 => Instr{ .PUSH = .HL },
        0xe6 => Instr{ .AND = Src8{ .Imm = mem.read(gb, pc + 1) } },
        0xe7 => Instr{ .RST = 0x20 },
        0xe8 => Instr{ .ADD_SP = mem.read(gb, pc + 1) },
        0xe9 => .JP_HL,
        0xea => Instr{ .LD_8 = .{ .dst = Dst8{ .Ind = as16(mem.read(gb, pc + 2), mem.read(gb, pc + 1)) }, .src = .A } },
        0xeb => Instr{ .INVALID = opcode },
        0xec => Instr{ .INVALID = opcode },
        0xed => Instr{ .INVALID = opcode },
        0xee => Instr{ .XOR = Src8{ .Imm = mem.read(gb, pc + 1) } },
        0xef => Instr{ .RST = 0x28 },

        0xf0 => Instr{ .LD_8 = .{ .dst = .A, .src = Src8{ .IndIoReg = mem.read(gb, pc + 1) } } },
        0xf1 => Instr{ .POP = .AF },
        0xf2 => Instr{ .LD_8 = .{ .dst = .A, .src = .IndC } },
        0xf3 => .DI,
        0xf4 => Instr{ .INVALID = opcode },
        0xf5 => Instr{ .PUSH = .AF },
        0xf6 => Instr{ .OR = Src8{ .Imm = mem.read(gb, pc + 1) } },
        0xf7 => Instr{ .RST = 0x30 },
        0xf8 => Instr{ .LD_16 = .{ .dst = .HL, .src = Src16{ .SPOffset = mem.read(gb, pc + 1) } } },
        0xf9 => Instr{ .LD_16 = .{ .dst = .SP, .src = .HL } },
        0xfa => Instr{ .LD_8 = .{ .dst = .A, .src = Src8{ .Ind = as16(mem.read(gb, pc + 2), mem.read(gb, pc + 1)) } } },
        0xfb => .EI,
        0xfc => Instr{ .INVALID = opcode },
        0xfd => Instr{ .INVALID = opcode },
        0xfe => Instr{ .CP = Src8{ .Imm = mem.read(gb, pc + 1) } },
        0xff => Instr{ .RST = 0x38 },
    };
}
