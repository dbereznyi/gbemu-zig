...
pub fn runPpu(gb: *Gb, cycles: usize) void {
    var rem_cycles = cycles + gb.ppu.cycles_odd;

    while (rem_cycles >= 4) {
        rem_cycles -= 4;

        // TODO simulate 1 T-cycle at a time
        stepPpu(gb);
    }

    gb.ppu.cycles_odd = rem_cycles;
}
        ...
        .drawing => {
            ...
            if (gb.ppu.dots % LINE_DOTS == DRAWING_START) {
                ...
                for (gb.ppu.x..gb.ppu.x + 4) |x| {
                    ...
                    gb.ppu.screen[gb.ppu.y * 160 + x] = gb.ppu.palette.data()[colorId];
                }
